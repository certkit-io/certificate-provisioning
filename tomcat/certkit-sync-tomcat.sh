...
if [[ ! -f "${DESTINATION_KEYSTORE_FILE}" ]]; then
  NEED_UPDATE=true
...
# Convert JKS if necessary
DEST_EXT="${DESTINATION_KEYSTORE_FILE##*.}"
if [[ "$DEST_EXT" == "jks" ]]; then
    # Convert .pfx to .jks
    keytool -importkeystore \
      -noprompt \
      -srckeystore "$SRC_PFX" -srcstoretype PKCS12 -srcstorepass "$CERTKIT_S3_SECRET_KEY" \
      -destkeystore "$SRC_JKS" -deststoretype JKS -deststorepass "$CERTKIT_S3_SECRET_KEY"
    SRC_KEYSTORE="$SRC_JKS"
else
    # Just keep the PFX as-is
    SRC_KEYSTORE="$SRC_PFX"
fi

# Even if nothing is 'new', apply if content differs
if files_differ "$SRC_KEYSTORE" "$DESTINATION_KEYSTORE_FILE"; then
  NEED_UPDATE=true
  REASONS+=("Key store differs from destination")
fi

if $NEED_UPDATE; then

  echo "Update required due to: ${REASONS[*]}"
  mkdir -p "$(dirname -- "$DESTINATION_KEYSTORE_FILE")"

  # Copy into place atomically where possible
  echo "Copying certificate files..."
  cp -f -- "$SRC_KEYSTORE" "$DESTINATION_KEYSTORE_FILE"
